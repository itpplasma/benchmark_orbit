...
def trace_orbit(vmec_file, initial_conditions, output_dir="run", trace_time=1e-3):
    ...
    print(f"    Expected dtaumin_initial: {2*3.14159*params.rt0/params.npoiper2:.2e}")
    
    all_orbits_data = []
    
    for orbit_idx, z0_vmec in enumerate(initial_conditions):
        print(f"\nOrbit {orbit_idx + 1}/{n_orbits}:")
        print(f"  Initial conditions: s={z0_vmec[0]:.3f}, theta={z0_vmec[1]:.3f}, phi={z0_vmec[2]:.3f}")
        ...
        print(f"  Final s: {s[-1]:.3f}, Energy conservation: {v_total[-1]/v_total[0]:.6f}")
    ...
    if n_orbits == 1:
        ...
        ds = xr.Dataset(
            {
                "s": (["time"], orbit_data["s"], {"long_name": "Normalized toroidal flux", "units": ""}),
                "theta_vmec": (["time"], orbit_data["theta_vmec"], {"long_name": "VMEC poloidal angle", "units": "rad"}),
                "phi_vmec": (["time"], orbit_data["phi_vmec"], {"long_name": "VMEC toroidal angle", "units": "rad"}),
                "theta_can": (["time"], orbit_data["theta_can"], {"long_name": "Canonical poloidal angle", "units": "rad"}),
                "phi_can": (["time"], orbit_data["phi_can"], {"long_name": "Canonical toroidal angle", "units": "rad"}),
                "v_total": (["time"], orbit_data["v_total"], {"long_name": "Total velocity", "units": "v_th"}),
                "v_parallel": (["time"], orbit_data["v_parallel"], {"long_name": "Parallel velocity", "units": "v_th"}),
                "v_perpendicular": (["time"], orbit_data["v_perpendicular"], {"long_name": "Perpendicular velocity", "units": "v_th"}),
                "pphi": (["time"], orbit_data["pphi"], {"long_name": "Canonical toroidal momentum", "units": ""}),
                "mu": (["time"], orbit_data["mu"], {"long_name": "Magnetic moment", "units": ""}),
                "B": (["time"], orbit_data["B"], {"long_name": "Magnetic field strength", "units": "T"}),
                "R": (["time"], orbit_data["R"], {"long_name": "Major radius", "units": "cm"}),
                "Z": (["time"], orbit_data["Z"], {"long_name": "Vertical coordinate", "units": "cm"}),
            },
            coords={
        ...
        # Initial conditions for each orbit
        initial_s = np.array([orbit["initial_conditions"][0] for orbit in all_orbits_data])
        initial_theta = np.array([orbit["initial_conditions"][1] for orbit in all_orbits_data])
        initial_phi = np.array([orbit["initial_conditions"][2] for orbit in all_orbits_data])
        
        ds = xr.Dataset(
            {
                "s": (["orbit", "time"], s_all, {"long_name": "Normalized toroidal flux", "units": ""}),
                "theta_vmec": (["orbit", "time"], theta_vmec_all, {"long_name": "VMEC poloidal angle", "units": "rad"}),
                "phi_vmec": (["orbit", "time"], phi_vmec_all, {"long_name": "VMEC toroidal angle", "units": "rad"}),
                "theta_can": (["orbit", "time"], theta_can_all, {"long_name": "Canonical poloidal angle", "units": "rad"}),
                "phi_can": (["orbit", "time"], phi_can_all, {"long_name": "Canonical toroidal angle", "units": "rad"}),
                "v_total": (["orbit", "time"], v_total_all, {"long_name": "Total velocity", "units": "v_th"}),
                "v_parallel": (["orbit", "time"], v_par_all, {"long_name": "Parallel velocity", "units": "v_th"}),
                "v_perpendicular": (["orbit", "time"], v_perp_all, {"long_name": "Perpendicular velocity", "units": "v_th"}),
                "pphi": (["orbit", "time"], pphi_all, {"long_name": "Canonical toroidal momentum", "units": ""}),
                "mu": (["orbit", "time"], mu_all, {"long_name": "Magnetic moment", "units": ""}),
                "B": (["orbit", "time"], b_field_all, {"long_name": "Magnetic field strength", "units": "T"}),
                "R": (["orbit", "time"], r_cyl_all, {"long_name": "Major radius", "units": "cm"}),
                "Z": (["orbit", "time"], z_cyl_all, {"long_name": "Vertical coordinate", "units": "cm"}),
                "initial_s": (["orbit"], initial_s, {"long_name": "Initial s coordinate", "units": ""}),
                "initial_theta": (["orbit"], initial_theta, {"long_name": "Initial theta coordinate", "units": "rad"}),
                "initial_phi": (["orbit"], initial_phi, {"long_name": "Initial phi coordinate", "units": "rad"}),
            },
            coords={
                "time": (["time"], all_orbits_data[0]["time"], {"long_name": "Time", "units": "s"}),
                "orbit": (["orbit"], np.arange(n_orbits), {"long_name": "Orbit index", "units": ""}),
            },
            attrs={
    ...
    if n_orbits == 1:
        output_file = os.path.join(output_dir, f"orbit_trace_{timestamp}.nc")
    else:
        output_file = os.path.join(output_dir, f"orbit_traces_{n_orbits}orbits_{timestamp}.nc")
...
if __name__ == "__main__":
    ...
    if len(initial_conditions) == 1:
        ...
        print(f"  s range (all orbits): {ds.s.min().values:.3f} - {ds.s.max().values:.3f}")
        ...
        print(f"  Energy conservation (min/max): {energy_conservation.min().values:.6f} / {energy_conservation.max().values:.6f}")
        print(f"  Mu conservation (min/max): {mu_conservation.min().values:.6f} / {mu_conservation.max().values:.6f}")